...
mf = flopy.modflow.Modflow(modelName, sim_ws=sim_ws)
...
recharge_AnnualAvg = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[-1] / 1000 / 365)
)
recharge_Jan = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[0] / 1000 / 31)
)
recharge_Feb = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[1] / 1000 / 28)
)
recharge_Mar = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[2] / 1000 / 31)
)
recharge_Apr = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[3] / 1000 / 30)
)
recharge_May = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[4] / 1000 / 31)
)
recharge_Jun = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[5] / 1000 / 30)
)
recharge_Jul = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[6] / 1000 / 31)
)
recharge_Aug = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[7] / 1000 / 31)
)
recharge_Sep = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[8] / 1000 / 30)
)
recharge_Oct = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[9] / 1000 / 31)
)
recharge_Nov = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[10] / 1000 / 30)
)
recharge_Dec = (
    np.ones((nrow, ncol), dtype=np.float32) * rchF * (rain_Data[11] / 1000 / 31)
)

rech_StressPeriod_dat = {
    0: recharge_AnnualAvg,
    1: recharge_Jan,
    2: recharge_Feb,
    3: recharge_Mar,
    4: recharge_Apr,
...
mt = flopy.mt3d.Mt3dms(
    modflowmodel=mf, modelname=modelName, ftlfilename="mt3d_link.ftl", sim_ws=sim_ws
)
...
nprs = (
    -1
)  # options: >0 save at "timprs", 0=only end step saved, <0 save at transport step
...
dsp = flopy.mt3d.Mt3dDsp(mt, al=alpha, trpt=ratio_ht, trpv=ratio_ht, dmcoef=DmCoeff)
...
ax1.plot(times, h_obs1, "-", c="c", label="Obs 1", markersize=mS, linewidth=lW)
ax1.plot(times, h_obs3, "-", c="r", label="Obs 2", markersize=mS, linewidth=lW)
ax1.plot(times, h_Spill, "-", c="b", label="Spill site", markersize=mS, linewidth=lW)
ax1.plot(times, h_obs4, "-", c="k", label="Obs 3", markersize=mS, linewidth=lW)
ax1.grid()
ax1.set_xlim(-10, 375)
# ax1.set_ylim(1.45,2)
...
ax2.plot(times, h_obs1, "-", c="c", label="Obs 1", markersize=mS, linewidth=lW)
ax2.plot(times, h_obs3, "-", c="r", label="Obs 2", markersize=mS, linewidth=lW)
ax2.plot(times, h_Spill, "-", c="b", label="Spill site", markersize=mS, linewidth=lW)
ax2.plot(times, h_obs4, "-", c="k", label="Obs 3", markersize=mS, linewidth=lW)
ax2.grid()
ax2.set_xlim(-10, 375)
# ax2.set_ylim(1.45,2)
...
ax3.plot(times, h_obs1, "-", c="c", label="Obs 1", markersize=mS, linewidth=lW)
ax3.plot(times, h_obs3, "-", c="r", label="Obs 2", markersize=mS, linewidth=lW)
ax3.plot(times, h_Spill, "-", c="b", label="Spill site", markersize=mS, linewidth=lW)
ax3.plot(times, h_obs4, "-", c="k", label="Obs 3", markersize=mS, linewidth=lW)
ax3.grid()
ax3.set_xlim(-10, 375)
# ax3.set_ylim(1.45,2)
...
cmax = 10**6
cs = pmv.plot_array(
    conc_30d, cmap=cmap, vmin=cmin, vmax=cmax
)  # concentration colourmap
cbar = plt.colorbar(cs, format="%.0e")  # colour bar
cbar.ax.set_ylabel("C [$\mu$g L$^-$$^1$]")

# plot spill site
ax1.plot(
    obsSpill_xcoord, obsSpill_ycoord, "o", c="r", label="Spill site", markersize=mS
)
...
cmax = 10**6
cs = pmv.plot_array(
    conc_365d, cmap=cmap, vmin=cmin, vmax=cmax
)  # concentration colourmap
cbar = plt.colorbar(cs, format="%.0e")  # colour bar
cbar.ax.set_ylabel("C [$\mu$g L$^-$$^1$]")
...
obs4_data = conc[:, :, int(obsSpill_row), int(obsSpill_col)][
    :, 0
]  # Observation at extraction well
...
ax.plot(times, obs1_data, "-^", c="r", label="obs1", markersize=mS, linewidth=lW)
ax.plot(times, obs2_data, "-s", c="k", label="Obs2", markersize=mS, linewidth=lW)
ax.plot(times, obs3_data, "-o", c="b", label="Obs3", markersize=mS, linewidth=lW)
ax.plot(times, obs4_data, "-o", c="g", label="spill point", markersize=mS, linewidth=lW)
